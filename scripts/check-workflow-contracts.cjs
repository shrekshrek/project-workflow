...
requireMarkers("docs/actions/README.md", [
  ...
  "Archive eligibility, current-truth merge, and physical lifecycle closure",
  "complete `feature-done` receipt",
]);
...
requireMarkers("README.md", ["Reused accepted FULL artifacts first pass the current mechanical-shape recheck"]);
requireMarkers("docs/workflow.md", ["reuse accepted full → current mechanical shape 复查", "通过后不重复主观 review"]);
requireMarkers("docs/project-workflow-overview.drawio", ["复用 accepted full 先复查 current shape", "当前合同切片", "completion preflight", "首次 terminal verdict", "完整 READY receipt", "后续显式调用", "approved patch + 验证摘要"]);
forbidMarkers("docs/project-workflow-overview.drawio", ["唯一端点入口(幂等)", "same-task partial recheck", "backlog/drift 自动入", "commit message 草稿"]);
requireMarkers("scripts/lib/fixture-contracts.cjs", ["validateFullFixtureAgainstCurrentPreflight", "Delivery Shape Baseline", "Prior decisions lacks", "plan/tasks slice IDs", "completion fixture contains unfinished tasks"]);
for (const relative of ["scripts/check-reviewer-fixtures.cjs", "scripts/check-feature-init-fixtures.cjs"]) {
  requireMarkers(relative, ["validateFullFixtureAgainstCurrentPreflight"]);
}
...
for (const relative of ["adapters/claude/skills/feature-done/SKILL.md", "adapters/codex/skills/feature-done/SKILL.md"]) {
  requireMarkers(relative, ["agents-md-reviewer", "spec-reviewer", "## Proof Bundle", "After required L1 passes", "Reviewers are read-only", "completion preflight", "not-run(completion preflight)", "later explicit invocation", "Lifecycle: READY; archive pending", "do not inline the full receipt"]);
  forbidMarkers(relative, ["lightweight architecture-drift observation", "architecture-drift observation"]);
}
requireMarkers("docs/actions/feature-done.md", ["Light-lane verification", "transient validation evidence", "receipt/history", "Resolved durable behavior with no existing area document is `update pending`", "area unresolved", "Persist only fields with a downstream consumer", "Completion Preflight", "primary-flow smoke", "mixed-feature", "Do not inline the full receipt"]);
...
requireMarkers("docs/actions/feature-done.md", ["Delivery Shape Baseline", "high-impact surfaces", "routes to `spec-revise`, lane upgrade, or a child feature", "without an accepted", "other sections", "do not substitute"]);
...
requireMarkers("docs/actions/feature-done.md", ["Every multi-boundary full-lane feature", "reused feature", "matching contract-bearing slice IDs in plan/tasks", "one-boundary feature is N/A", "omitting slice declarations does not claim", "A missing", "mapping or incomplete slice returns `NEEDS WORK`"]);
...
requireMarkers("docs/actions/feature-done.md", ["review-cycle snapshot", "Git/non-Git changed-path population", "exact applicable", "unverified-item-count=0", "blocking-ambiguity-count=0", "A PASS never persists applicable IDs or populations", "do not persist its path list or a population/content hash", "dependency closure"]);
requireMarkers("docs/actions/feature-done.md", ["available only in a later explicit `feature-done` invocation", "one revised snapshot", "original full-population terminal evidence", "fix is limited to the cited findings and their dependency closure", "change outside the finding/dependency closure", "unaffected changed-path population", "A later task", "base=<commit SHA>", "before writing those endpoint-owned outputs", "reviewed=<commit SHA>; dirty=no", "reviewed=worktree; dirty=yes", "Other reviewed/dirty pairings are invalid", "target-branch merge base", "ambiguous repository, base, or feature boundary blocks", "Derive changed paths from Git", "For light lane, run it only when", "The only allowed output difference is this action's declared receipt/status write"]);
requireMarkers("docs/actions/feature-done.md", ["Each explicit invocation dispatches at most one applicable L2/L3 population", "parallel L2/L3 roles together count as that one review cycle", "A terminal `NEEDS WORK` ends the invocation", "never auto-fixes or redispatches", "at most one L2/L3 dispatch cycle", "later explicit invocation"]);
...
requireMarkers("docs/actions/feature-done.md", ["Validate the complete owner-supplied review package", "one representation for every Verification obligation", "validates package structure, presence, and readability", "explicit gap or failed mapped evidence is a finding", "record every applicable review slot", "not-run(review-package incomplete)", "invalidated(review-input drift)", "never auto-start another full-population", "Stop the current invocation"]);
...
requireMarkers("docs/actions/feature-archive.md", ["archive-eligible only", "required fields defined by the", "`Verdict: READY`", "Checks", "Review execution", "Current truth", "ineligible", "rerun feature-done", "eligible only as a current-task result", "does not re-anchor it", "not proof that the reviewed worktree was unchanged", "delivery evidence and current truth as separate freshness questions", "later movement of a current branch or PR head does not by itself invalidate", "validate every pending current-truth fact against present implementation evidence"]);
...
requireMarkers("README.md", ["Same-task focused re-review requires a later explicit `feature-done` invocation"]);
...
requireMarkers("docs/actions/feature-init.md", ["For `Feature: reuse=<path>`", "derive the sequence from that artifact's lane and lifecycle", "accepted full first rechecks", "current mechanical artifact-shape prerequisites", "draft full begins with", "without another subjective review", "A reused artifact follows its own lane/lifecycle gates"]);
requireMarkers("docs/actions/feature-init.md", ["no `{{TODO}}` enters implementation", "concrete bounded implementation-time task/decision", "owner, boundary, and evidence", "without routine section-by-section confirmation", "Preserve unknowns as TODOs"]);
...
requireMarkers("docs/reviewers/spec-quality-reviewer.md", ["Every test layer or matrix required by the artifact", "fails Q3", "deleted or consolidated", "test counts", "smallest sufficient", "missing or incomplete Delivery Shape Baseline", "do not substitute for it", "accepted spec content needs no duplicate"]);
...
requireMarkers("docs/spec-driven.md", ["只把需要持久追踪的选择", "选择性当场写回", "普通实现细节不逐项记账", "不能替代当前 Prior decisions schema 或 Delivery Shape Baseline", "需要持久 why/source", "N/A(no durable why/source decision)` 不属于反模式"]);
...
requireMarkers("docs/actions/feature-init.md", ["scope-viability check", "Never report a nonblocking Scope Viability result", "clarification-required", "split-required", "bundled-risk-accepted", "pending-selection", "pending-handoff", "size alone never requires a split", "durable decomposition handoff", "explicitly untracked outcomes", "Do not create a repository backlog", "separate external write that requires the user's explicit authorization", "Recheck them at `spec-quality-check`"]);
requireMarkers("docs/actions/feature-init.md", ["### Implementation Scope Stop", "before", "production code", "tests, migrations, compatibility paths", "Safe read-only", "necessary-detail", "simpler implementation", "compact `Scope stop`", "Ask at most one", "FULL is not permission"]);
requireMarkers("docs/actions/feature-init.md", ["### Implementation Continuation Check", "Every multi-boundary full-lane feature", "contract-bearing slices", "same slice IDs", "one-boundary full-lane feature", "implement the current slice", "focused L1", "compact progress update", "Do not dispatch L2/L3", "same failure repeats without new diagnostic evidence", "context", "compaction", "continue without reconfirmation", "scope-growth triggers", "tier, directory, file count, test layer/case, or estimated duration", "feature, action, lane, gate"]);
...
requireRegex("docs/actions/feature-init.md", /Apply the Impact, necessity[\s\S]{0,700}Decide whether a new artifact[\s\S]{0,900}Apply Scope Viability/, "impact before artifact decision before scope viability");
...
for (const relative of ["adapters/claude/skills/feature-init/SKILL.md", "adapters/codex/skills/feature-init/SKILL.md"]) {
  requireMarkers(relative, ["canonical impact, necessity, scope-viability", "ordinary single-outcome paths silent", "docs/architecture-design.md", "ordinary features skip it", "decision-completeness-auditor", "Reviewer execution"]);
}
...
for (const relative of ["adapters/claude/skills/feature-init/SKILL.md", "adapters/codex/skills/feature-init/SKILL.md"]) {
  requireMarkers(relative, [
    ...
    "`PREVIEW` is",
    "compatible active feature is reused",
    "canonical compact route result",
    "DIRECT/LIGHT/FULL gate sequence",
  ]);
}
...
for (const relative of ["adapters/claude/skills/feature-done/SKILL.md", "adapters/codex/skills/feature-done/SKILL.md"]) {
  requireMarkers(relative, ["current full-lane artifact shape", "completed slices and focused L1", "Git/scope/Baseline", "explicit Guidance", "not-run(completion preflight)"]);
  forbidMarkers(relative, ["reused legacy work"]);
  requireMarkers(relative, ["Each explicit invocation performs at most one", "L2/L3", "ends on its first terminal verdict", "do not repair or redispatch", "later explicit invocation"]);
...
requireMarkers("template/docs/specs/changes/_template/plan.md", ["按真实依赖顺序", "多边界 FULL feature", "稳定 slice ID", "tasks.md 使用相同 ID", "单边界 FULL 不为形式硬拆", "合同切片", "Implementation Continuation Check", "material delta", "Implementation Scope Stop", "下一个 consumer", "最小 focused evidence", "large/extra-large"]);
...
requireMarkers("docs/actions/feature-init.md", ["Keep outcomes, scope, constraints", "exclusions in `spec.md`", "do not copy them into plan Prior decisions", "non-obvious", "not a raw transcript", "before `spec-quality-check`"]);
for (const relative of ["adapters/claude/skills/feature-init/SKILL.md", "adapters/codex/skills/feature-init/SKILL.md"]) {
  requireMarkers(relative, ["Carry Scope Stop and the Continuation Check", "contract-bearing slice", "focused L1", "task/progress update", "boundary recheck", "Do not add artificial slices", "L2/L3", "Proof Bundle", "dependent work"]);
  requireMarkers(relative, ["reused accepted FULL", "current mechanical-shape recheck", "new/draft FULL", "spec-quality-check"]);
}
...
requireMarkers("template/docs/specs/changes/_template/tasks.md", ["plan §5 slice ID 一一对应", "实现 → focused L1 → 勾选", "Implementation Continuation Check", "不要求用户", "不运行 L2/L3", "单边界 FULL 不为形式硬拆", "Setup / migration / composition", "映射 spec §4 obligation", "只列尚未由 slice focused L1", "N/A(all spec §4 obligations mapped in slices)", "不要重复 checklist"]);
...
for (const relative of ["adapters/claude/skills/spec-quality-check/SKILL.md", "adapters/codex/skills/spec-quality-check/SKILL.md"]) {
  requireMarkers(relative, ["accepted outcome and impact boundary", "`Implementation Scope Stop`", "smallest-"]);
  requireMarkers(relative, ["matching", "plan/tasks slices for multi-boundary full-lane work", "`Implementation Continuation Check`", "focused/final evidence rules into implementation"]);
  requireMarkers(relative, ["missing or incomplete Delivery Shape Baseline", "other artifact sections do not substitute"]);
  requireMarkers(relative, ["Q6 Guidance Placement", "root/tier/module/mechanical", "difference-only", "one-line alias"]);
}
requireMarkers("docs/actions/spec-quality-check.md", ["Every multi-boundary full-lane implementation", "matching contract-bearing slice IDs", "one-boundary full-lane feature is not artificially split", "focused L1", "inspectable exits", "compact progress", "must be repaired before `READY`", "Scope,", "do not substitute for it", "Implementation Scope Stop and Implementation Continuation Check", "without reconfirmation", "material mismatch stops dependent work"]);
requireMarkers("docs/reviewers/spec-quality-reviewer.md", ["Multi-boundary full-lane", "matching plan/tasks slice IDs", "one-boundary feature is not artificially split", "focused L1", "inspectable exit", "next consumer", "context/session resume", "plan/tasks mismatch", "inspectable point before endpoint review", "missing or incomplete Delivery Shape Baseline", "do not substitute for it"]);
...
requireMarkers("docs/actions/spec-quality-check.md", ["Every required Prior decisions entry uses three semantic", "decision, why, and source", "`决策 | 为什么 | 来源`", "equivalent localized header is valid", "missing semantic column, row source, or explicit N/A", "mechanical failure", "including a complete `Delivery Shape Baseline`", "Constraints, or Module Impact do not substitute for it"]);
requireMarkers("docs/actions/spec-quality-check.md", ["Q7d: the artifact records", "decision/why/source semantic columns", "source in every required row"]);
...
requireMarkers("docs/examples/reviewer-mutation-smoke.md", ["Remove the Delivery Shape Baseline", "regardless of artifact age", "module-impact/scope sections must not substitute"]);
requireMarkers("docs/examples/reviewer-mutation-smoke.md", ["Prior decisions semantic source column", "mechanical gate returns", "before reviewer dispatch", "N/A(no durable why/source decision)"]);
requireMarkers("docs/examples/reviewer-mutation-smoke.md", ["outside the accepted Delivery Shape Baseline", "justified only by possible future use", "Q7c", "Q7d", "Requirements-reconciliation smoke", "MISMATCH: missing-from-artifact", "MISMATCH: unsupported-artifact", "MISMATCH: superseded-remnant", "MISMATCH: cross-artifact-conflict", "SOURCE GAP", "no second dispatch"]);
...
requireMarkers("docs/examples/reviewer-mutation-smoke.md", ["Implementation Continuation Check smoke", "accepted medium, multi-boundary full-lane outcome", "resume after context compaction", "without new diagnostic evidence", "no L2/L3 or Proof Bundle", "contradicts the accepted failure-state contract", "backend/frontend/test", "one real contract boundary", "large/extra-large", "independently acceptable, enableable, and revertible"]);
requireMarkers("docs/examples/reviewer-mutation-smoke.md", ["real dependency order inside each named", "internal coding order as a tier/file/test/time bucket", "omit the multi-boundary", "remains `BLOCKED` until plan/tasks", "must not repeat all slice checks as bookkeeping"]);
...
for (const relative of [
  ...
  "adapters/claude/skills/spec-quality-check/SKILL.md",
  "adapters/codex/skills/spec-quality-check/SKILL.md",
]) forbidMarkers(relative, ["pre-3.11", "legacy-boundary fallback", "legacy-unambiguous", "a new artifact records", "older artifact without the current Source", "table-shape migration"]);
requireMarkers("docs/spec-driven.md", ["每节填完简洁同步并继续", "不要为常规 section transition 请求确认", "一次问一个阻塞问题并等待", "不重复列一遍"]);
forbidMarkers("docs/spec-driven.md", ["OK 进下一节?", "至少列 2-3 条"]);
requireMarkers("docs/workflow.md", ["同一失败重复且没有新诊断证据"]);
forbidMarkers("docs/workflow.md", ["同一个错改 3 次没好"]);
requireMarkers("docs/quickstart.md", ["accepted full 先复查当前 mechanical artifact shape", "通过后继续实施并最终走 `feature-done`", "不通过则修复/revise 后走 `spec-quality-check`", "当前 slice 实现 → focused L1", "切片内不运行 L2/L3", "每次显式调用最多一轮适用 L2/L3", "不会在同一调用内形成修复—复审循环"]);
...
requireMarkers("docs/actions/feature-archive.md", ["required fields defined by the", "`Verdict: READY`", "Checks", "Review execution", "Current truth", "missing or invalid required", "ineligible", "never infer READY", "current-task dirty-worktree READY", "reuse across tasks", "ordinary filesystem rename", "move the directory back", "rerun `feature-done`"]);
requireMarkers("adapters/claude/skills/feature-archive/SKILL.md", ["canonical archive-eligibility receipt field", "rerun `feature-done`", "exact Git/non-Git identity and freshness rules", "pending current truth", "ordinary filesystem rename", "move the directory back"]);
requireMarkers("adapters/codex/skills/feature-archive/SKILL.md", ["canonical archive-eligibility receipt field", "rerun `$feature-done`", "exact Git/non-Git identity and freshness rules", "pending current truth", "ordinary filesystem rename", "move the directory back"]);
for (const relative of [
  "README.md",
...
]) forbidMarkers(relative, ["receipt-schema migration", "migration candidate", "review-scope", "legacy fields", "older receipt"]);
for (const relative of [
  "docs/actions/feature-done.md",
  "template/docs/specs/changes/_template/tasks-light.md",
  "tests/fixtures/reviewer-smoke/light-base/docs/specs/changes/002-normalize-key-light/tasks.md",
