...
At every applicable boundary, use an independent reviewer when the host can dispatch one. Main-session fallback
is valid when dispatch is unavailable or capacity is exhausted, provided the same read-only role contract and
evidence are used. Invoking the owning action requires no extra workflow confirmation; host security approvals
still apply.

Independent reviewer boundaries may run in parallel or sequentially. Scheduling does not change their inputs,
evidence requirements, or verdict contract.
...
A reviewer never edits the reviewed implementation or artifacts. After the gate returns, an enclosing
implementation request may apply an unambiguous in-scope fix, but only a later explicit user request invokes
the owning gate against a fresh snapshot; material decisions return to the user.
