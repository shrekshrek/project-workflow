...
- **Light lane**: `tasks.md` goal/boundary (including constraints) and verification; no separate spec is required
- full-lane `plan.md` and `tasks.md` as context
...
Detect `brownfield` from `## Delta` / `## Motivation`, `greenfield` from `## 1. Outcomes`, or `light` from the tasks-only artifact. Fresh-read and extract testable items only from:

| Shape | Extract from |
|---|---|
| Greenfield | Outcomes, Scope (do + don't), Constraints, Verification |
| Brownfield | Delta subsections, Constraints, Verification |
| Light | Goal/boundary, constraints, verification in `tasks.md` |
...
Return verdict, feature/baseline/shape, reviewed-scope identity, a coverage declaration (`complete` or the concrete
