...
- `clean`: `feature-done 001-normalize-key` returns `READY`; L2/L3 zero-finding evidence lists exact changed paths, every applicable rule/spec-item ID, no applicable-but-unverified items, and no blocking ambiguity; the receipt stays compact.
...
- Record case, adapter, endpoint verdict, actual cited findings, exact applicable/unverified IDs, ambiguities, and the exact persisted receipt in the release PR/task. A static CI pass is not endpoint evidence.
