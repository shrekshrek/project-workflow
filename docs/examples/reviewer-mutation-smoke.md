...
- `clean`: `feature-done 001-normalize-key` returns `READY`; L2/L3 zero-finding evidence lists exact changed paths and exact rule/spec-item IDs with 100% applicable coverage; the receipt stays compact.
...
- Record case, adapter, endpoint verdict, actual cited findings, coverage/confidence, and exact persisted receipt in the release PR/task. A static CI pass is not endpoint evidence.
