...
[feature-init](actions/feature-init.md) 拥有沟通、记录需要、授权、上下文和范围暂停规则；普通明确工作直接开发，多个可独立交付的结果先自然建议拆分并由用户选择。[spec-quality-check](actions/spec-quality-check.md) 检查依据是否足以开始。文档内容是否有用比章节是否齐全重要，独立审查按风险和项目要求决定。

协作默认不假设用户熟悉仓库、实现或测试术语，但保留用户对业务结果、风险和成本的判断权。先说明用户会看到什么、哪些选择会改变结果，再按需解释技术名词。开发前的验证用于解决会改变方向的关键未知，不默认要求 E2E 或完整回归；已有证据足够时不为流程重复试验。

### 3.2 实现阶段:机械细节不中断,方向风险及时沟通

普通实现细节和任务细化由 AI 按已有约定处理。影响范围、方案、成本、数据、权限或验收的新发现，先暂停相关实施、说明影响与建议，用户确认后局部更新并继续。已写代码不构成扩大范围的理由。

切片内连续处理普通实现、预期内修复和聚焦检查，不增加逐文件审批；中途出现可能改变方向或判断的实质问题，立即暂停受影响工作。完成有意义的业务纵切片时，按风险运行最短的用户路径 smoke，并交接原目标、实际结果、证据、偏差和下一步，等待用户再进入下个切片。小任务不人为拆片。开发中的检查是实现反馈，不冒充最终交付证明。先回应当前问题，在自然节点写回有用结论；精确执行边界只见 [feature-init](actions/feature-init.md#implementation-scope-stop)。

### 3.3 交付阶段:delivery receipt

[feature-done](actions/feature-done.md) 在一个稳定快照上检查实际改动、L1、适用的独立 L2/L3 和当前事实，汇总真实证据。交付阶段才根据接受义务、关键用户旅程、实际改动面和项目要求选择必要的集成、E2E、回归或发布检查；E2E 是一种风险证据，不是固定配额。non-READY 结束本轮；允许的直接修复作为聚焦切片，交接并等待后才重入完整 gate。新记录的收据位于 spec.md，外部报告引用它，不复制整段日志。
