...
│   │   └── security.md
│   ├── hooks/
│   │   └── lint-on-edit.cjs        # 骨架(显式 CommonJS,不受项目 type:module 影响)
│   └── settings.json
├── .codex/                         # Codex adapter runtime enforcement
│   ├── hooks.json                  # PostToolUse hook mapping
│   └── hooks/
│       └── lint-on-edit.cjs        # thin wrapper over .claude/hooks/lint-on-edit.cjs
├── docs/
│   ├── adr/                        # 架构决策记录
│   │   ├── README.md
│   │   └── 0000-template.md
...
P0 时先落 hook adapter。只有已确认存在 <5 秒、支持单文件参数且不会扩大写范围的命令时才激活,做到**文件保存后自动 lint/format**;否则明确保留 `scaffold/inactive`,端点 L1 仍由 `feature-done` 跑全量 command。详细的 hook 设计哲学见 §4.2。
...
- hook active 时改一个匹配文件,验证 lint/format 真执行;scaffold/inactive 时报告原因,不能声称自动强制已生效
...
Canonical [`spec-revise` action](actions/spec-revise.md) 定义修订 SOP:先停实施,用 ADR 记录决定,同步 spec revision record、plan 和 tasks,完成审计与用户批准后再恢复实施。不得偷偷改冻结 spec、边改契约边改代码,或把全部修订拖到交付时一次性补写。只是措辞澄清且不改变契约时,写 plan prior decision 即可。
