...
- **不是 process-owning 框架**:不强制每步用工具或某个命令
- **不是某个 AI 编程工具的专用文档**:Claude Code / Codex / OpenCode / Cursor 都适用
- **不是某个栈的专属指南**:跨语言、跨技术栈的通用方法论
- **不承诺"AI 一次写对"**:核心命题是**消除"对齐劳动"**(反复提醒 AI 注意规范、命名、结构的对话内耗),不追求"零迭代"或"几乎免费"(详见 §0.4 / §0.5)
...
│ ─ 工具:Project Setup skill(跨工具,在已开的 AI session 内调用) │
├─────────────────────────────────────────────────────────────┤
...
> **关于 Module Setup**:早期版本把"P1 Module Setup"列为独立阶段,实践证明**模块几乎不独立发生**,几乎总是 P2 feature 的子产物。本手册保留 §2 章节做详细解释,但**它是 P2 的 sub-flow,不是独立 phase**。
...
**B. 工程化层**(栈相关,参考 [`scaffold-v2/`](../scaffold-v2/) 直接 fork 或参照写)
...
> **重要重述**:早期版本把这个叫"P1 Module Setup"列为独立阶段。**实践证明这是错的** —— 模块几乎从不独立发生,**几乎 100% 是 P2 feature 的副产物**。本节保留章节做详细解释,但所有触发 / 产出 / 校验都嵌在 P2 流程中,**P0 之后直接进 P2**。
...
| 模式 | 触发 | 问什么 |
|---|---|---|
| **B. 主动 refresh** | `/refresh-agents-md` 命令 | 对比 AGENTS.md vs 实际状态,问"这些 drift 要写进去吗?" |
| **C. 端点反思** | 模块/功能完成时(P3 端点) | "刚做的工作有 N 处可能值得沉淀,要写进 AGENTS.md?" |
| **D. 信号触发** | hook 检测"记得 X" 出现 >= 2 次 | "你最近多次提醒 X,这是项目级约定要常驻吗?" |
...
>
> **参考实现**:[`scaffold-v2/`](../scaffold-v2/)(Vue 3 + Element Plus + FastAPI 全栈,可直接 `pnpm bootstrap && pnpm dev`)。
>
> **栈级陷阱**:[`docs/gotchas.md`](gotchas.md) —— 10 条 scaffold-v2 验证过程中沉淀的具体踩坑(Docker / asyncpg / Pydantic extras / 测试基建)。
