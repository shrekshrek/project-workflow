...
2. **主力 Claude Code,Claude Code / OpenCode / Codex 基本通用即可,不强求"完全工具无关"**。
   ...
   **底层逻辑**:底层工具是 weeks-级别迁移成本,上层规范(AGENTS.md / spec.md)是 months-级别投入;**上层规范要尽量 portable**,但不要为了拼凑"理论上 N 个工具都能跑"而稀释主力工具的体验。详见 §7.6 反模式 + [`docs/tooling.md §5 本项目实际选择`](tooling.md#5-本项目实际选择--理由)。
