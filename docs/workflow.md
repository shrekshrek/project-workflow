...
**单步可拆**:`/feature-done` 是 L1+L2+L3+proof-bundle 的 default orchestrator。Claude Code adapter 保留 [`/l1-review`](../skills/l1-review/SKILL.md) / [`/l2-review`](../skills/l2-review/SKILL.md) / [`/l3-review`](../skills/l3-review/SKILL.md) / [`/proof-bundle`](../skills/proof-bundle/SKILL.md) 作为 helper surfaces,用于局部复查、缓存复用或调试;它们不是独立 workflow action。
...
| **B. 端点反思**(顺手) | feature 完成时 | `feature-done` 写 proof bundle Item 5a(已应用)+ Item 5b(待 backlog);Claude helper [`/proof-bundle`](../skills/proof-bundle/SKILL.md) 可单独修补 |
...
**落地**:此规则由 `feature-done` 的装配步骤执行;Claude Code 的 [`proof-bundle`](../skills/proof-bundle/SKILL.md) helper 也遵守同一规则。Reviewer 各自报独立 finding,**去重在 aggregator**,不下沉到 reviewer 内部(否则违反 §6.4 主张的"正交问题正交工具")。
