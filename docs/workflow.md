...
| **功能级** | `docs/specs/<NNN>-<slug>/{spec,plan,tasks}.md` 或轻车道 `tasks.md`;活动区只放**进行中**,已交付的由 `feature-archive` 整目录移入 `docs/specs/archive/` | 仅需要持久 feature artifact 的任务创建(`/feature-init`) |
| **产品域级**(扁平,可选) | `docs/current/<area>.md`(+ 可选 `docs/specs/index.md` 编号索引) | 仅长周期、多轮演进的产品域出现时创建(首次 `feature-archive`);不预置 |
...
| **B. 任务**(Specs) | `docs/specs/<NNN>-<slug>/{spec,plan,tasks}.md` 或轻车道 `tasks.md`;交付收尾后整目录移入 `docs/specs/archive/` | "这个 tracked feature **做什么 + 怎么做 + 步骤**?" | per-feature 生命周期,完成后**物理归档**为历史(`feature-archive` 清扫) | 仅需要持久 artifact 的任务创建;小修直接做 |
...
| **E. 产品事实**(Current truth) | `docs/current/<area>.md`(可选 `docs/specs/index.md` 索引) | "这个产品/系统域**现在**怎么工作?"(未来 feature 的第一参考) | 当前态;由 `feature-archive` / `spec-reconcile` 在交付后更新 | 仅长周期、多轮方向演进的产品域;首次 `feature-archive` 时创建,不预置 |
...
"想看 feature 设计"          → B(docs/specs/<NNN>-<slug>/;已交付的在 docs/specs/archive/,只当历史读)
"想看为什么选 X"             → C(docs/adr/NNNN-<X>.md)
"想看 hook / 工具配置"        → D(.claude/hooks/, .claude/settings.json, .codex/hooks.json)
"想看某产品域现在的样子"      → E(docs/current/<area>.md;没有 E 时读最新 spec 但警惕过时)
```
...
/project-workflow:spec-quality-check
 ...
 Fail       Pass
   ...
   │     ├─ current-truth check(仅当 docs/current/<area>.md 存在:是否矛盾 / 是否需更新)
   ...
   │          → (若有 pending)合并结论进 docs/current/<area>.md
   │          → 已交付 feature 整目录 git mv → docs/specs/archive/;被取代老 spec 标 已取代/已废弃(spec-driven.md §5)
   │
...
**局部复查怎么做**:`/feature-done` 是 L1+L2+L3+proof-bundle 的唯一端点入口,不再拆分独立的 helper 命令。需要单独重跑某一层时:L1 直接跑项目 check 命令;L2 / L3 在主会话直接 dispatch [`agents-md-reviewer`](../agents/agents-md-reviewer.md) / [`spec-reviewer`](../agents/spec-reviewer.md) sub-agent;proof bundle 修补则重跑 `/feature-done`(幂等,复用有效缓存)。
...
这个任务是否需要新的 project-workflow artifact?
  改行为且触达的域**已有 docs/current/<area>.md**? → 无论 diff 多小,至少轻车道
    (current truth 的唯一写入口在管线上,绕过管线的行为变更会让它静默腐化;
     没建 current truth 的域不受此限,小行为改动照常直接做)
