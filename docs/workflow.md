...
切片内连续处理普通实现、预期内修复和聚焦检查，不增加逐文件审批；中途出现可能改变方向或判断的实质问题，立即暂停受影响工作。完成有意义的切片时，运行最小相关检查，交接后等待用户再进入下个切片。小任务不人为拆片。开发中的检查是实现反馈，不冒充最终交付证明。先回应当前问题，在自然节点写回有用结论；精确执行边界只见 [feature-init](actions/feature-init.md#implementation-scope-stop)。
