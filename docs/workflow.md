...
| **C. 决策**(ADR) | `docs/adr/NNNN-<title>.md` | "**当时为什么**这么选?+ trade-off?" | 时间戳化快照,append-only | 重大架构选择 / spec-revise 满足 `ADR_REQUIRED` / 模块边界变更时(§3.5 / §2.6)|
| **D. 工具基础设施**(Infra) | `.gitignore` + 条件式 adapter hooks/settings | "工具**自动跑**什么?" | 跟项目演化 | 有可靠命令才创建 |
...
**轮 2 — 主语言**(跨 tier 共性 auto-derive,user 可 override)← 影响 paths / 命令推导
- 主语言?(若多语言列 2-3 个)
...
4. **按复杂度触发,不按 finding 数触发**:生产中连续零 findings 不能证明 auditor 有效或无效。简单同步用 inline trace;复杂生成用 subagent。Sensitivity 由 known-bad mutation smoke 证明。
...
| **Coverage 指标** | 输出 `coverage = fully_verified_applicable / applicable × 100%`;definite nonmatches 不进分母;零 findings 的 high confidence 需 100% 且无 applicable-unverified / blocking ambiguity | Verification(覆盖率可量化)|
