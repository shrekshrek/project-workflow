...
**v2 主力支撑**:[§6.2 Context budget](#62-上下文是有限预算context-budget) —— AGENTS.md 行数纪律 + `.claude/rules/` 按需加载 + `@imports` 组织长尾 + `/clear` / `/compact` + 小 composable skills
...
│ ─ /feature-init <slug> → spec.md + plan.md + tasks.md       │
...
这张表回答 "**文件长哪、在树里哪里**";紧接的 4 类表回答 "**文件回答什么问题**"。两表正交,合起来形成完整心智模型。
...
| **功能级** | `docs/specs/<NNN>-<slug>/{spec,plan,tasks}.md` | 每开发任务一份(`/feature-init` 创建) |
| **路径级约定**(扁平 + globs frontmatter) | `.claude/rules/*.md`(如 `code-style.md` / `testing.md` / `security.md`) | P0 默认含若干 topic,文件触发匹配的 globs 时按需加载(见 [§1.6](#16-路径级规则claude-rules官方支持)) |
...
| **工具基础设施**(扁平,不嵌套) | `.claude/{hooks,settings.json}` + `.gitignore`(+ drift 状态:`.claude/refresh-ignore` / `.claude/drift-ledger.json`) | P0 默认创建,后续按需扩展 |
...
|---|---|---|---|---|
| **A. 约定**(Conventions) | `AGENTS.md`(项目 / tier / 模块 3 级嵌套)+ `.claude/rules/*.md`(扁平 + globs 路径触发)+ `CLAUDE.md` 1 行 alias | "我们**现在**怎么工作?"(规则 / 风格 / 最佳实践) | 当前态;AGENTS.md 3 层频率梯度见 [§1.3](#13-agentsmd-的内容标准),`.claude/rules/` 见 [§1.6](#16-路径级规则claude-rules官方支持) | 项目级必然;tier / 模块级可选 |
| **B. 任务**(Specs) | `docs/specs/<NNN>-<slug>/{spec,plan,tasks}.md` | "这个 feature **做什么 + 怎么做 + 步骤**?" | per-feature 生命周期 | 每开发任务一份(`/feature-init` 创建)|
| **C. 决策**(ADR) | `docs/adr/NNNN-<title>.md` | "**当时为什么**这么选?+ trade-off?" | 时间戳化快照,append-only | 重大架构选择 / spec-revise / 模块边界变更时(§3.5 / §2.6)|
| **D. 工具基础设施**(Infra) | `.claude/{hooks,settings.json}` + `.gitignore` | "工具**自动跑**什么?" | 跟项目演化 | P0 默认创建,后续按需扩展 |

**两轴交叉规则**:
- 代码组织维度只影响 **A. 约定** 中 AGENTS.md 部分的嵌套层数(根 / tier / 模块);`.claude/rules/` 是 A 类但**扁平**,靠 frontmatter globs 实现路径触发
- B / C / D 三类与代码组织维度独立,**不按 tier / 模块嵌套**
...
"想看代码风格 / 测试 / 安全"  → A(.claude/rules/<topic>.md)
"想看 feature 设计"          → B(docs/specs/<NNN>-<slug>/)
"想看为什么选 X"             → C(docs/adr/NNNN-<X>.md)
"想看 hook / 工具配置"        → D(.claude/hooks/, .claude/settings.json)
```
...
2. **主力 Claude Code,Claude Code / OpenCode / Codex 基本通用即可,不强求"完全工具无关"**。

   AGENTS.md / spec.md 这层用最广可读的格式(markdown + 标准约定),让 OpenCode / Codex 基本能读;Cursor / Gemini CLI 等其他工具**不在 v2 目标内** —— 顺手能用更好,读不了 v2 也不为此牺牲 Claude Code 上的功能深度(plugin / skills / hooks 等)。

   **底层逻辑**:底层工具是 weeks-级别迁移成本,上层规范(AGENTS.md / spec.md)是 months-级别投入;**上层规范要尽量 portable**,但不要为了拼凑"理论上 N 个工具都能跑"而稀释主力工具的体验。详见 §7.6 反模式 + [`docs/tooling.md §2 底层 IDE/CLI`](tooling.md#2-底层ide--cli-层)(主用工具选择理由)。

3. **Plugin / skill 工具的角色:scaffold + 条件性框架问 + 提醒 + 兜底,不当 interviewer 替 user 决策业务细节**。

   Plugin 应该做的:
   - **Scaffold** ── 起骨架(spec/plan/tasks 模板 / AGENTS.md 多层 / `.claude/rules/`)
   ...
   **底层逻辑**:这是 §0.4 "不依赖反复人工提醒" + §0.5 信念 #1 "对齐对话在系统 - AI 之间"在 plugin tool 层的落地。**关键区分:framework decision(必问)vs business detail(conversational fill + gate)** ── 准则是"audit 能否 cheaply catch + user 是否当下能答 + 延后成本"。详见 [§6.3 规则由环境强制](#63-规则由环境强制environment-enforced-rules) + `/feature-init` SKILL.md 实现。

**做对了之后的副产物**(不追求,但顺带得到):
- 迭代成本自然下降 —— 系统接管机械合规后,你不必盯每一步
- AI 输出更稳定 —— 规范常驻 → 输入更清晰 → 输出收敛
- 项目跨人 / 基本跨工具传递成本低 —— 上层 markdown 资产可读性强(但工具切换本身不是 v2 优化目标)
...
│   # docs/specs/<NNN>-<slug>/ 由 /feature-init 按需创建;
...
### 1.3 A 类约定的内容标准(AGENTS.md + `.claude/rules/`)

A 类约定有两个载体——**AGENTS.md**(嵌套 / 全局加载)+ **`.claude/rules/<topic>.md`**(扁平 / 路径触发加载)。本节先讲各自的内容标准,再给"写哪边"的决策口诀。机制层面(globs frontmatter / debug)见 [§1.6](#16-路径级规则claude-rules官方支持)。
...
#### AGENTS.md vs `.claude/rules/<topic>.md`:写哪边?

| 维度 | AGENTS.md | `.claude/rules/<topic>.md` |
|---|---|---|
...
| **决策口诀** | 任何文件都要看的 → AGENTS.md | 只有某类文件要看的 → `.claude/rules/` |
...
**A 类反模式**:
- "任何文件都要看的项目级约定"塞进 `.claude/rules/<topic>.md` —— 只有读匹配 globs 的文件时才加载,其他时候 AI 看不到 → 漂移
- framework-specific 详规则全塞 AGENTS.md → 突破 < 200 行,所有 session 全部加载 → context budget 爆炸
- 同一规则在 AGENTS.md + `.claude/rules/` 两边重复 → 删 AGENTS.md 那份(后者更精准 path-scoped)
- 新建 `.claude/rules/<topic>.md` 但忘设 globs(无 globs → 失去 path-scoped 优势,等于扁平 always-on)

> **A 类不止 P0 写**:AGENTS.md(3 层)和 `.claude/rules/` 各自的**更新频率与触发跨 P2/P4**,AGENTS.md 详见 [§5.0 三层 AGENTS.md 的更新频率梯度](#50-三层-agentsmd-的更新频率梯度);`.claude/rules/` 多在 P2 内当 framework 模式提炼出来时新增/扩充(扁平,没 tier/模块嵌套梯度)。
...
> Anthropic 还支持**系统级** `/etc/claude-code/CLAUDE.md`(全机器范围,企业 IT/DevOps 强制策略用)——v2 audience(个人 / 小团队)碰不到,本手册不列入主线。
...
**为什么用**:把"长尾内容"(完整架构文档、共享规则)拆出去,主文件保持 < 100 行;AI 读时仍加载 import 内容。它节省的是主文件复杂度和维护成本,不是已加载后的 token 成本;真正的按需 context 节省主要来自 `.claude/rules/` 的 path-scoped 加载、skills 和主动 `/clear` / `/compact`。
...
>
> project-workflow v2.3+ 已把 floor 收紧到 Claude Code only(见 [docs/tooling.md §2](tooling.md#2-底层ide--cli-层) IDE/CLI 选择)。**默认**:rules 放 `.claude/rules/<topic>.md` + `globs:` frontmatter,享 path-scoped 加载;`/project-init` 模板自动按此结构生成。`.claude/rules/` 是 Claude-private 目录,其他 AI 工具(Cursor / Codex / OpenCode)不读 —— 本 plugin 不再为跨工具优化,如有需要请绕开 plugin 自行管理。
...
**ADR 与其他文档的关系**:4 类文档(A 约定 / B 任务 / C 决策 / D 基础设施)总框架见 [§0.3 文档职责 4 类](#03-概念区分钉死再读后续)。ADR 是 C 类,**唯一回答"当时为什么这么选"**——跟 AGENTS.md(A 类规则)/ spec.md(B 类 WHAT)/ plan.md(B 类 HOW)分工正交,不重叠。
...
**立场**:v2 P0 starter **默认不预置** `.github/ISSUE_TEMPLATE/` 或 `.github/PULL_REQUEST_TEMPLATE.md`。原因:
...
```
[P0 完成] AGENTS.md / .claude/rules/ 已就位
   │
   ▼
─────────── 规划阶段(§3.1)───────────
...
/project-workflow:spec-quality-check
 ...
 Fail       Pass
   ...
   │     ├─ L1 重检(hooks 一次性回扫)
   │     ├─ L2 review(agents-md-reviewer 拿 AGENTS.md + .claude/rules/ 作 context)
...
| `/l3-review`(被 `/feature-done` 调用) | **实施后端点** | **代码**做了 spec 说要做的事吗(code-vs-spec drift) |

**单步可拆**:`/feature-done` 是 L1+L2+L3+proof-bundle 的 one-shot orchestrator。也可分步用 [`/l1-review`](../skills/l1-review/SKILL.md) / [`/l2-review`](../skills/l2-review/SKILL.md) / [`/l3-review`](../skills/l3-review/SKILL.md) / [`/proof-bundle`](../skills/proof-bundle/SKILL.md) 单独跑。
...
```
改动跨 3+ 文件 OR 涉及架构?
  YES → 起新功能目录 docs/specs/<NNN>-<slug>/,写 spec.md + plan.md(全道)
  NO  → 轻车道:仅 tasks.md(目标/边界 + 验证 + proof,免 frozen spec/plan),见 spec-driven.md §3.2.5;键盘级 5 行改直接做(§7.4)
...
> 上面是**行动决策**(要不要起 spec / 要不要 Sibling Alignment)。spec/plan **写完后**的**质量自检**(7 问 checklist)见 [`spec-driven.md §3.7`](spec-driven.md#37-specplan-写完后的质量自检7-问-checklist) —— 实施开始前必跑。

**写完 spec.md / plan.md 后,实施前必跑 [`spec-driven.md §3.7` 7 问 quality checklist](spec-driven.md#37-specplan-写完后的质量自检7-问-checklist)**(或 `/spec-quality-check` skill)—— 本节决策清单是"要不要写 spec";§3.7 是"已经写完的 spec 够不够好"。两步互补,都不可省。
...
3. Review summary —— 调用 pr-review-toolkit / 同类 reviewer
                     必须提供两份 context:
                       · A 类约定:AGENTS.md(3 层)+ 命中本 feature 路径的 .claude/rules/*.md(L2 项目约定合规)
...
**proof bundle 的载体**:`docs/specs/<NNN>-<slug>/tasks.md` 末尾的 `## Proof Bundle` 节(模板已预留)。完成时填,不必走 PR 流程。

> 团队 / 外部协作场景:可自行加 `.github/PULL_REQUEST_TEMPLATE.md`,内容同 5 项 —— 见 [§1.9](#19-平台协作v2-默认不铺模板)。v2 默认不预置。
...
> **范围**:P4 **只动 A 类约定**(`AGENTS.md` 多层嵌套 + `.claude/rules/*.md`)。其他文档各有自己的演化通道:
>
> | 文档 | 是否在 P4 | 演化通道 |
> |---|---|---|
> | **A 类约定**(`AGENTS.md` + `.claude/rules/*.md`) | ✅ 是 | 本节 —— 跨 feature 持续累积、定期主动 refresh(工具:[`/project-workflow:agents-md-revise`](../skills/agents-md-revise/SKILL.md)) |
...
- **~~信号触发 hook~~**:🚫 **v2 不实施** —— hook 自动检测 "记得 X" 重复并主动 nudge 跟 [§0.5 信念 1](#05-实现策略的核心信念)("消除对齐劳动")**相悖**(系统主动提示本身就是新对齐对话源),且模式识别误报率高。用户感知 drift → 走"主动 refresh" 即可
...
| **B. 端点反思**(顺手) | feature 完成时 | [`/proof-bundle`](../skills/proof-bundle/SKILL.md) Item 5a(已应用)+ Item 5b(待 backlog);由 `/feature-done` 端点自动调 |

> 历史上还有 "模式 C 信号触发 hook" —— **v2 不实施**,理由见 §5.1 注。

### 5.3 工具流程概览

详细 step 见 [`/agents-md-revise` SKILL.md](../skills/agents-md-revise/SKILL.md)。5 步骨架:

1. **读 A 类约定全集**:`AGENTS.md` 多层嵌套 + `.claude/rules/*.md`,提取每条可验证 statement
...
- **蓝图侧**(Blueprint —— 本项目 v2 提供):skill / template / hook 配置 / proof bundle workflow 等
...
|---|---|---|---|
| **6.1** Spec-driven | `/feature-init` 三文件模板 + `/project-init` skill(P0) + Refresh skill(P4) | 真去写、维护 spec | ~50 / 50 |
...
规约(spec)是 source of truth / 行为契约,代码是 AI 根据契约生成和修改的实现产物。任何 AI 协作的跨文件改动,**必须有 spec 兜底**。
...
| CLAUDE.md / AGENTS.md | 主体 < 200 行;共享长尾可用 `@imports` 组织,路径相关长尾优先放 `.claude/rules/` 做按需加载 |
...
**修正**:协作约定写在**广泛可读**的位置(AGENTS.md / `docs/`,主力 Claude Code 但 OpenCode / Codex 也能读),工具特定位置只放工具特异配置(如 `.claude/hooks/` / `.claude/settings.json`)。v2 不强求"工具完全无关",但要避免锁死单一工具的私有格式 —— 见 [§0.5 信念 2](#05-实现策略的核心信念)。
...
- [OpenAI Symphony](https://github.com/openai/symphony) —— manage work, not agents;对 v2 的工作交接状态、可接手交付思想有启发,但 proof bundle 是本 workflow 的落地综合
