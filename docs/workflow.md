...
**问题**:AI 没有跨会话长期记忆,规范会朝三个方向漂——**时间**(同代码不同 session 评判不同)、**空间**(模块 A 跟模块 B 风格不一致)、**演进**(项目第 1 月跟第 6 月代码风格差)。多 agent / 多 session 并行开发进一步放大空间漂移:并行实例之间不共享记忆,A 类约定(AGENTS.md + path-scoped rules)同时承担跨实例协调协议的角色。
...
│ ─ Q&A → baseline(AGENTS.md/CLAUDE.md/rules/specs;hooks 按需)│
...
│ ─ 后端先行(全栈适用)→ 实施 → proof bundle → archive 收尾    │
...
| **路径级约定**(扁平 + 路径匹配) | core 语义是 path-scoped rules;Claude adapter 默认 materialize 为 `.claude/rules/*.md`(如 `code-style.md` / `testing.md` / `security.md`) | P0 默认含若干 topic;Claude 用 `paths:` YAML 列表触发按需加载(见 [§1.6](#16-路径级规则claude-rules官方支持));其他 adapter 映射到自己的规则载体 |
...
**两轴交叉规则**:
- 代码组织维度只影响 **A. 约定** 中 AGENTS.md 部分的嵌套层数(根 / tier / 模块);path-scoped rules 是 A 类但**扁平**,由各 adapter 映射到自己的路径匹配机制(Claude 为 `.claude/rules/` + `paths:` YAML 列表)
...
| **D 基础设施** | `project-init` / `project-personalize` 只在命令 active + verified 时创建 | 当普通 repo 基础设施变更处理:按风险直接改、走 light 或 full lane,并验证真实执行 | 不设专用周期 action;失效 hook 修复或删除,不能留 no-op mapping |
...
├── .claude/rules/                  # Claude adapter 对 path-scoped rules 的 materialization
│   ├── code-style.md
│   ├── testing.md
│   └── security.md
├── .claude/hooks + settings.json   # 仅 active + verified 时 materialize
├── .codex/hooks*                   # 同一 verified command 的 Codex mapping,否则不存在
├── docs/
...
>
> Claude Code adapter 默认把路径级规则放在 `.claude/rules/<topic>.md` + `paths:` YAML-list frontmatter,享 path-scoped 加载;无 `paths:` 的 rule 全局加载。`/project-init` 模板自动按此结构生成。`.claude/rules/` 是 Claude-private 目录,其他 AI 工具(Codex / OpenCode / Cursor)不原生读取。跨工具迁移时不要复制一份新方法论,而是把同一套 A 类约定映射到目标 adapter 的规则 / hook 载体;具体边界见 [`cross-tool-methodology.md`](cross-tool-methodology.md)。
...
Q&A 只收生成 baseline 必需的信息:项目类型与 tier、主语言、框架和真实 package/test/lint 选择;fullstack 再按 service/UI tier 补各自特有选型。已从对话或 manifests 确认的内容不重复问,mixed-language 才拆成 per-tier 命令。精确问题顺序和宿主交互以 canonical [`project-init` action](actions/project-init.md)及 runtime skill 为准,本文只保留设计取舍。
...
L2 / L3       —— verdict + exact applicable IDs + coverage/unverified/ambiguity/confidence evidence
...
Canonical verdict 由 [`feature-done` action](actions/feature-done.md) 定义:L1 失败、必要上下文缺失或检查无法可靠运行 = `BLOCKED`;L1 已通过但仍有可修的 L2/L3/current-truth finding 或证据缺口 = `NEEDS WORK`;全部闭环 = `READY`。本文不复制判定表,避免与 action 漂移。
...
> | 文档 | 是否在 P4 | 演化通道 |
> |---|---|---|
> | **A 类约定**(`AGENTS.md` + path-scoped rules) | ✅ 是 | 本节 —— 客观 drift 出现时主动 refresh(工具:[`/project-workflow:agents-md-revise`](../skills/agents-md-revise/SKILL.md)) |
...
| 模式 | 触发 | 工具 |
|---|---|---|
| **A. 主动 refresh** | 用户感知到 drift / 发现客观不一致 / 大依赖升级后 | [`/project-workflow:agents-md-revise`](../skills/agents-md-revise/SKILL.md) —— 扫客观 drift,逐条 yes/no/ignore-forever,apply + commit 草稿 |
| **B. 端点反思**(顺手) | feature 完成时 | `feature-done` 把可行动但未沉淀的约定写入 delivery receipt `Drift` + drift ledger;需修补时重跑 `feature-done`(幂等) |
...
L2/L3 都遵守 cite-or-skip、fresh read、全量 applicable coverage 和 ambiguity feedback:没有规则引用或没有完整验证就不能给高置信空报告。精确输入、输出、coverage 公式与阻断条件只在 canonical [`agents-md-reviewer`](reviewers/agents-md-reviewer.md)和 [`spec-reviewer`](reviewers/spec-reviewer.md)定义,避免三处复制同一契约。
...
**症状**:门只输出模板套话,没有 exact scope/applicable population、coverage、unverified、ambiguity 或 confidence evidence
...
| Claude `/init` 交互流程可用 | 当前官方文档已记录 `CLAUDE_CODE_NEW_INIT=1`,但 project-workflow 不依赖该可选入口保证输出 |
...
- **project-workflow v3**:本手册是它的核心文档;`template/` 存放 P0 starter assets 与 feature 模板,`skills/` / Codex plugin skills 承载 actions,`scripts/` 负责 release checks
