...
Upgrade migration: a `## Proof Bundle` without `Verdict:`, or a READY receipt without the current `git=[base=...; reviewed=...; dirty=...]` / non-Git `inputs=[...]` reviewed-state identity (including the earlier `review-scope` or prose `base/worktree` schema), is a **receipt-schema migration candidate**. Sweep mode lists it separately and offers to rerun `feature-done`; never infer READY from fields that do not establish a valid reviewed state.
