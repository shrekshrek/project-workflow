...
- `L2` / `L3` for full lane: verdict, reviewed baseline sources, findings, applicable-but-unverified identifiers, and ambiguities. The baseline is the consulted convention files for L2 and the spec path plus shape-specific section set for L3. Reviewers must still return exact applicable identifiers to the endpoint so it can validate complete coverage; the persisted PASS receipt does not duplicate that transient enumeration. Expand cited findings, unverified identifiers, and ambiguities whenever any are non-empty.
