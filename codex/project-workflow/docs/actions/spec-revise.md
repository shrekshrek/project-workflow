...
6. When the revision materially expands scope, rerun the `feature-init` scope-viability check before drafting. If it introduces another independently acceptable, shippable, and revertible outcome without mandatory coupling, ask the user to create a child feature or explicitly accept the bundled-delivery risk. When the outcome is deferred to a child, require its stable issue/PM reference and record the handoff in plan Prior decisions before completing the revision; do not create a repository backlog or rely on chat memory. Do not draft the expansion before that decision and handoff; record an accepted bundle in the existing plan prior-decisions or risks section.
