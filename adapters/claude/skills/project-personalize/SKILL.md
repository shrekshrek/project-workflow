...
- Inspect with Read/Grep/Glob/Bash. Dispatch `codebase-explorer` only for a nontrivial structure survey. Dispatch `tech-researcher` only when a material stack, library, or tool choice remains unresolved and needs current external evidence; the user makes the final choice. If either named agent is unavailable, follow its canonical reviewer contract in the main session.
...
- Use an inline trace for simple synchronization; dispatch `decision-completeness-auditor` only at its canonical boundary.
