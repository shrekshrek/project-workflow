...
- Before implementation, explain outcome, approach, boundary, rationale and verification in plain language that requires no repository or testing knowledge while preserving the user's decision authority. Reuse an accepted approach; carry each functional outcome through its ordinary implementation and checks, report useful progress, stop for a material discovery, then hand off evidence and wait before the next outcome.
