...
- Execute the canonical preflight and gate with Bash, including its mechanical Git boundary, L1 evidence reuse,
  stable snapshots, bounded repair, review scheduling, repair-delta reconciliation, and execution fallback. A stop
...
- Capture the reviewed Git identity before endpoint-owned writes, then persist and structurally re-read the canonical `## Proof Bundle` receipt. For a non-READY verdict, map each blocker or blocker group to the canonical `Next` route before the enclosing workflow decides whether existing authorization permits repair. Return the action's concise human summary with `Lifecycle: READY; archive pending` when applicable and a repository-relative link to `tasks.md#proof-bundle`; the full receipt remains on disk.
