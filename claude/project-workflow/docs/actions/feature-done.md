...
L2/L3/Drift finding counts live in the delivery receipt; do not add a duplicate gate-health block. Repeated zero findings are a cost/calibration signal only. Inspect history only when the user requests calibration. Reviewer sensitivity is established with the repeatable [known-bad endpoint smoke](https://github.com/shrekshrek/project-workflow/blob/main/docs/examples/reviewer-mutation-smoke.md), not production silence.
