...
**模板源**:`$PLUGIN_ROOT/template/docs/specs/_template/{spec,plan,tasks}.md`(3 个文件,内容跟 [spec-driven.md §3.3](../../docs/spec-driven.md) 一致)。Plugin canonical 是 single source of truth ── 不支持 per-project template 定制(真有需求 fork plugin)。
...
→ **不预设 Q&A** 是 design choice ── feature 类型多样,固定 Q&A 必有 CRUD bias / 颗粒度 mismatch / caller plant 缺口;conversational mode 让 AI 据 user 真实业务上下文适应性 fill,更自然。**结合 chat-as-context pre-fill** 让 user 避免重复填已讨论内容。
...
- **Template source**:canonical 模板在 `$PLUGIN_ROOT/template/docs/specs/_template/{spec,plan,tasks}.md`(Step 5 直接 cp)。**不支持 per-project template 定制** ── 真有需求 fork plugin clone 改 `template/`
