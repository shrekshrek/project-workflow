...
**Not for**:A 类约定**创建**(→ `/project-init` 或 `/project-personalize`)/ in-feature 调整(→ `/spec-revise`)/ per-feature backlog(→ `/feature-done` Item 5b)/ code vs 约定合规检查(→ `/feature-done` L2)/ 产品 spec 生命周期与 current truth 维护(→ `/feature-archive` / `/spec-reconcile`;本 skill 只管 A 类工程约定)/ **主观模式信号 / hook 自动触发**(v2 不做,[workflow §0.5](../../docs/workflow.md#05-实现策略的核心信念))。
