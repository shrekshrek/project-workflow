...
MATERIALIZER="${PLUGIN_ROOT:+$PLUGIN_ROOT/scripts/materialize-project-baseline.cjs}"
if [ -z "$PLUGIN_ROOT" ] || [ ! -d "$PLUGIN_ROOT/template" ] || [ ! -f "$MATERIALIZER" ]; then
  MATERIALIZER="$(find "$HOME/.claude/plugins/cache" "$HOME/.codex/plugins/cache" -type f -path '*/project-workflow*/scripts/materialize-project-baseline.cjs' -print0 2>/dev/null | xargs -0 ls -t 2>/dev/null | grep '/scripts/materialize-project-baseline.cjs$' | head -1)"
  PLUGIN_ROOT="${MATERIALIZER%/scripts/materialize-project-baseline.cjs}"
fi
[ -n "$PLUGIN_ROOT" ] && [ -d "$PLUGIN_ROOT/template" ] && [ -f "$MATERIALIZER" ] || { echo "Cannot resolve a compatible project-workflow package with baseline materializer"; exit 1; }
