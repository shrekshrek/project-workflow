...
> 写“已声明行为/风险 + 最小可执行证据”,不是预排测试用例。不要从通用测试习惯派生未声明输入、边界或错误 case。一个证据可覆盖多个相关风险;只有交互维度、已有回归要求或发布/合规契约确实改变结果时才使用矩阵。用户可见 outcome 在已有义务中恰好标一个 `Primary flow`;它只决定 preflight 顺序,不新增第二套 smoke。**本节即 L3 review 基线。**
