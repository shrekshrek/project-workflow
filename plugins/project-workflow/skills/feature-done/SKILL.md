...
4. Run L2 against root/nested `AGENTS.md`, applicable scoped rules, and included gotchas. The reviewer must cite the convention source for every finding.
5. For full lane, run L3 against the change spec: greenfield Outcomes/Scope/Constraints/Verification; brownfield Delta/Constraints/Verification. Domain docs are context, not the L3 baseline. For light lane, record the explicit L3 skip.
...
7. Write a proof bundle to `tasks.md` containing diff summary, L1/L2/L3 evidence, convention changes/drift suggestions, current-truth state, and open questions. Record blocked/skipped layers explicitly rather than omitting them.
