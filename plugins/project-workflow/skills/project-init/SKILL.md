...
0. Resolve the target directory from the skill argument or cwd. If it already has `AGENTS.md`, redirect to `$project-personalize`. If it is this plugin source repository, warn and require confirmation.
...
3. Copy the starter template into the target, excluding `_multi_tier_examples/`, `.claude/rules/_examples/`, and feature/domain template directories. Copy the bundled `docs/gotchas.md` into the generated project.
4. Render root `AGENTS.md`, path-scoped rules, hook configuration, and any tier-level `AGENTS.md`. Keep `CLAUDE.md` files as one-line aliases. Generated active files must contain no unresolved `{{...}}` placeholders. Inventory generated `.claude/rules/` through the Codex bridge and surface ambiguous scope metadata.
...
7. Validate placeholders, rule frontmatter, bridge scope resolution, real path prefixes, command consistency, alias files, and AGENTS.md line counts. Report deferred commands and the global/matched/skipped/ambiguous compatibility-rule sets explicitly.
