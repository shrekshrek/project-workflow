...
**v2 主力支撑**:[§6.2 Context budget](#62-上下文是有限预算context-budget) —— AGENTS.md 行数纪律 + path-scoped rules 按需加载(Claude: `.claude/rules/`) + `@imports` 组织长尾 + `/clear` / `/compact` + 小 composable skills
...
| **工具基础设施**(扁平,不嵌套) | Claude adapter 默认 `.claude/{hooks,settings.json}` + `.gitignore`(+ drift 状态:`.claude/refresh-ignore` / `.claude/drift-ledger.json`) | P0 默认创建,后续按需扩展;其他 adapter 使用自己的 hook / config 目录 |
...
| 层 | 负责什么 | 是否绑定工具 | 例 |
|---|---|---|---|
| **Methodology core** | 流程、不变量、workflow action、reviewer 方法、文档契约、review 分层 | 否 | `AGENTS.md`, `docs/actions/`, `docs/reviewers/`, `docs/specs/`, ADR, proof bundle, L1/L2/L3 |
...
**外部工具**(可选):Anthropic 内置 / ECC / Superpowers 各有 brainstorming skill,选你顺手的或直接用 AI 主会话 —— v2 不强制。
...
> **跨工具口径**:`docs/specs/`、`docs/adr/`、`AGENTS.md` 是目标项目的 portable core 文件。路径级规则的 portable core 是"哪些规则作用于哪些路径"这个语义;`.claude/rules/` 只是 Claude adapter 的默认实现。Codex / OpenCode / Cursor 应映射到自己的规则载体,而不是把 `.claude/rules/` 当成方法论本体。
...
**单步可拆**:`/feature-done` 是 L1+L2+L3+proof-bundle 的 default orchestrator。Claude Code adapter 保留 [`/l1-review`](https://github.com/shrekshrek/project-workflow/blob/main/skills/l1-review/SKILL.md) / [`/l2-review`](https://github.com/shrekshrek/project-workflow/blob/main/skills/l2-review/SKILL.md) / [`/l3-review`](https://github.com/shrekshrek/project-workflow/blob/main/skills/l3-review/SKILL.md) / [`/proof-bundle`](https://github.com/shrekshrek/project-workflow/blob/main/skills/proof-bundle/SKILL.md) 作为 helper surfaces,用于局部复查、缓存复用或调试;它们不是独立 workflow action。
...
```
是否跨模块 / 跨职责边界,或涉及架构、数据模型、API/schema 契约、迁移、权限、安全、不变量路径?
  YES → 起新功能目录 docs/specs/<NNN>-<slug>/,写 spec.md + plan.md(全道)
  NO  → 同一模块内的小改可走轻车道:仅 tasks.md(目标/边界 + 验证 + proof,免 frozen spec/plan),见 spec-driven.md §3.2.5;键盘级局部改直接做(§7.4)
...
| 层 | 规则来源 | 检查什么 | 检查机制 | 时机 |
|---|---|---|---|---|
| **L1 机械层** | tool config(lint/type/test)+ 语言/团队通用卫生规则 | 代码机械合规吗?(lint/type/test/format) | hook(保存时单文件 lint+format,**自动改**)+ `feature-done` 端点全量 check(Claude helper `/l1-review` 可单独重跑) | 保存后 + 端点 |
...
| **B. 端点反思**(顺手) | feature 完成时 | `feature-done` 写 proof bundle Item 5a(已应用)+ Item 5b(待 backlog);Claude helper [`/proof-bundle`](https://github.com/shrekshrek/project-workflow/blob/main/skills/proof-bundle/SKILL.md) 可单独修补 |
...
规约(spec)是 source of truth / 行为契约,代码是 AI 根据契约生成和修改的实现产物。任何有 blast radius 的 AI 协作改动,**必须有可追踪 feature artifact 兜底**:全道用 `spec.md` / `plan.md` / `tasks.md`;轻车道小改用 `tasks.md` 的目标 / 边界 / 验证 / proof;键盘级局部改可直接做。
...
| 场景 | 动作 |
|---|---|
| 跨模块 / 跨职责边界 OR 数据模型 OR API/schema 契约 OR 迁移 / 权限 / 安全 / 不变量路径 | 必写 spec(`docs/specs/<NNN>-<slug>/spec.md`)|
...
| 类型 | 内容 |
|---|---|
| 官方结构 | Claude Code 把规则分为 `~/.claude/rules/`(用户级)、`CLAUDE.md` alias(项目/子目录级)和 docs/specs/(功能级);project-workflow 把 portable source 收敛到 `AGENTS.md` + path-scoped rules,再由 adapter 映射到工具文件 |
...
| 层 | 工具 | 时机 |
|---|---|---|
| L1 | hook(eslint/ruff/gofmt,保存时单文件 lint+format,**自动改文件**)+ endpoint check(默认由 `feature-done` 跑;Claude helper `/l1-review` 可单独重跑)| 保存后 + 端点 |
| L2 | reviewer agent + AGENTS.md 作 context | 端点(P3 proof bundle) |
| L3 | reviewer agent + spec.md 作 context + 测试 | 端点(P3 proof bundle) |

**组合在端点 action**:`feature-done` 是默认组合点,顺序聚合 L1 / L2 / L3 / proof bundle 并给出单一 verdict。Claude Code 可以保留 `/l1-review` / `/l2-review` / `/l3-review` / `/proof-bundle` 作为 helper surfaces,用于局部复查、缓存复用或调试;其他 adapter 不需要逐个复制这些 helper。
...
**落地**:此规则由 `feature-done` 的装配步骤执行;Claude Code 的 [`proof-bundle`](https://github.com/shrekshrek/project-workflow/blob/main/skills/proof-bundle/SKILL.md) helper 也遵守同一规则。Reviewer 各自报独立 finding,**去重在 aggregator**,不下沉到 reviewer 内部(否则违反 §6.4 主张的"正交问题正交工具")。
...
- **project-workflow v2**:本手册是它的核心文档,`template/` 存放 P0 starter assets 与插件内 feature 模板,tools/ 是 P0/P4 工具
