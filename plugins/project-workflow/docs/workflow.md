...
| **功能级** | `docs/specs/<NNN>-<slug>/{spec,plan,tasks}.md` 或轻车道 `tasks.md` | 仅需要持久 feature artifact 的任务创建(`/feature-init`) |
...
| **B. 任务**(Specs) | `docs/specs/<NNN>-<slug>/{spec,plan,tasks}.md` 或轻车道 `tasks.md` | "这个 tracked feature **做什么 + 怎么做 + 步骤**?" | per-feature 生命周期 | 仅需要持久 artifact 的任务创建;小修直接做 |
