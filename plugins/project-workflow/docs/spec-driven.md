...
> [README](https://github.com/shrekshrek/project-workflow/blob/main/README.md) 里 spec-driven 主题的展开。
>
...
**工具**:[`/project-workflow:spec-quality-check`](../skills/spec-quality-check/SKILL.md) 自动化本 7 问 checklist——机械检查(M1-M5)+ dispatch [`spec-quality-reviewer`](reviewers/spec-quality-reviewer.md) sub-agent 做主观二审(Q4 Outcomes / Q5 Constraints / Q7 verifiable)。**实施前 gate**——pass / borderline / fail 三档 verdict + 修法建议;failed 阻断实施,borderline 需要显式记录风险。
...
**关于 spec.md 状态字段**(`> 状态: 草稿 / 已确认 / 已实现`,template 默认有):是**业务流程标签**,跟本节编辑边界**正交**。**草稿** = 仍可自由迭代;**已确认** = 用户接受并开始实施,契约冻结;**已实现** = 契约已被代码兑现(spec 自身的兑现标记,不是部署状态)。部署 / 上线状态由 CI / 部署系统跟踪,不在 spec 上标(故无"已上线")。

> **谁翻**:`feature-done` 给 READY 时翻(契约兑现的判定点);Claude helper `/proof-bundle` 单独修补证据时也可执行同一动作。只动状态标记不动契约,不走修订 SOP。
...
| L3 spec 合规局部复查 | [`/l3-review`](https://github.com/shrekshrek/project-workflow/blob/main/skills/l3-review/SKILL.md) —— Claude helper surface;dispatch [`spec-reviewer`](reviewers/spec-reviewer.md) agent,按 §1 Outcomes / §2 Scope / §3 Constraints / §4 Verification 分组报告 |
