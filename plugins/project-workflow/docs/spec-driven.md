...
| **已 commit 且开始 impl** | **必走 SOP**(`## 修订记录` 节追加 + 跨文件同步;满足 `ADR_REQUIRED` 时加 ADR)| [`/spec-revise`](../skills/spec-revise/SKILL.md) |
...
**反模式**:把 commit 前的 iteration 改动当 frozen 后修订处理(走完整 revision 流程),或给每次 frozen 修订无条件起 ADR → ceremony 过度,spec 反而难起步。**没 commit 前就是 draft,改它不算修订**。
